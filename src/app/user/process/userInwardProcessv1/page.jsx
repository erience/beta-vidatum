...
const UserInwardProcessv1 = () => {
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);
  const apiUrl = process.env.NEXT_PUBLIC_API_BASE_URL;

  //----------------------fetch inwardprocess v1 data-----------------------------
  const fetchInwardProcessData = async () => {
    try {
      const response = await apiConnector(
        "GET",
        `${apiUrl}/v2/user/process/inward-indexv1`
      );
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        accessorKey: "count",
        cell: (info) =>
          info.row.original.count ? info.row.original.count : "0",
      },
      {
        header: "Status",
        accessorKey: "status",
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            <>
              <Link
                href={`/user/process/viewInwardProcessv1/${btoa(
                  rowData.id
                )}`}
...
export default UserInwardProcessv1;
