...
const UnlistedIsinRegister = () => {
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);
  const apiUrl = process.env.NEXT_PUBLIC_API_BASE_URL;

  //----------------------fetch UnlistedIsinRegister data-----------------------------
  const fetchUnlistedRegisterData = async () => {
    try {
      const response = await apiConnector(
        "GET",
        `${apiUrl}/v2/admin/register/unlisted-isin-register`
      );
      ...
      },
      {
        ...
        cell: (info) => {
          ...
          switch (status) {
            ...
            case 4:
              return (
                <span className="inline-block text-white bg-gray-500  text-xs font-semibold px-3 py-1 rounded-full">
                  Status 4
                </span>
              );
            default:
    ...
    <>
      <div className="card">
        <div className="card-body">
          <ViewDataTable
            title="Unlisted ISIN Register"
            cardHeader="Unliasted isin register data"
            columns={columns}
            data={data}
            isLoading={loading}
          />
...
export default UnlistedIsinRegister;
