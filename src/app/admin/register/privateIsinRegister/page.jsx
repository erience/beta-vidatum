...
const PrivateIsinRegister = () => {
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);
  const apiUrl = process.env.NEXT_PUBLIC_API_BASE_URL;

  //----------------------fetch PrivateIsinRegister  data-----------------------------
  const fetchPrivateRegisterData = async () => {
    try {
      const response = await apiConnector(
        "GET",
        `${apiUrl}/v2/admin/register/private-isin-register`
      );
    ...
    <>
      <div className="card">
        <div className="card-body">
          <ViewDataTable
            title="Private ISIN Register"
            cardHeader="private isin register data"
            columns={columns}
            data={data}
            isLoading={loading}
          />
...
export default PrivateIsinRegister;
