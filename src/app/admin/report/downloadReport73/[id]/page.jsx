...
const DownloadReport73 = () => {
  const { id } = useParams();
  const [data, setData] = useState({});
  const [isinData, setIsinData] = useState({});
  const apiUrl = process.env.NEXT_PUBLIC_API_BASE_URL;

  const fetchReport73Data = async () => {
    try {
      const response = await apiConnector(
        "GET",
        `${apiUrl}/v2/admin/report/download_report73/${id}`
      );
    ...
    <div className="w-full flex justify-center print:text-[11px] print:leading-tight print:h-[1122px] print:w-[794px] print:overflow-hidden">
      <div className="w-full max-w-[794px] bg-white p-6 print:p-4 text-[13px] print:text-[10px]">
        ...
        <div className="text-center mb-4">
          <p className="mb-1">
            <strong>Subject:</strong> In the matter of Regulation 7(3) of SEBI
        ...
        <div className="text-justify leading-tight">
          ...
          <p className="mb-2">
            * The SEBI has mandated all listed companies that with effect from
            April 1, 2019, requests for effecting transfer of securities should
            not be processed in the physical form. [the SEBI (Listing
            Obligations and Disclosure Requirements) (Fourth Amendment)
            Regulations, 2018 & Press Release No. 49/2018]
          </p>
          <p className="mb-2">
            <b>Note:</b> The company has changed the Share transfer agent w.e.f
            March 16, 2022.
          </p>
          <p className="mb-2">
...
export default DownloadReport73;
