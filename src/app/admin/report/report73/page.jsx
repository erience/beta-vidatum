...
import formatPeriodToDMY, { formatDateShort, formatDMY } from "@/utils/helper";
...
const Report73 = () => {
  ...
  //----------------------fetch report73 data-----------------------------
  const fetchReport73 = async () => {
    try {
      const response = await apiConnector(
        "GET",
        `${apiUrl}/v2/admin/report/report73`
      );
  ...
  const checkAddressFunction = async (isin, id) => {
    try {
      ...
      if (data.status) {
        ...
        const url = `/admin/report/downloadReport73/${btoa(id)}`;
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        cell: (info) => (info.row.original.is_download ? "Yes" : "No"),
      },
      {
        header: "Note",
        accessorKey: "notes",
        cell: (info) =>
          info.row.original.notes ? info.row.original.notes : "-",
      },
      {
        header: "Status",
        accessorKey: "status",
    ...
    <>
      <div className="card">
        <div className="card-body">
          <ViewDataTable
            title="Upload Report 73"
            cardHeader="Upload Report 73"
            columns={columns}
            data={data}
            isLoading={loading}
          />
