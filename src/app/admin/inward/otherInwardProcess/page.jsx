...
const OtherInwardProcess = () => {
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);
  const apiUrl = process.env.NEXT_PUBLIC_API_BASE_URL;
  const [activeYear, setActiveYear] = useState(null);

  //----------------------fetch OtherInwardProcess data-----------------------------

  const fetchOtherInwardData = async () => {
    try {
      const response = await apiConnector(
        "GET",
        `${apiUrl}/v2/admin/process/inward/others-inward-process?table=b_inward&isin=b_isin`
      );
    ...
    <>
      <div className="card">
        <div className="card-body ">
          ...
          <ViewDataTable
            title="Other Inward Process"
            cardHeader="Other inward Process"
            columns={columns}
...
export default OtherInwardProcess;
