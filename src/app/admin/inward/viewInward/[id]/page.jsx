...
const ViewInward = () => {
  ...
  const fetchInwardData = async () => {
    try {
      const response = await apiConnector(
        "GET",
        `${apiUrl}/v2/admin/viewInward?ref=${id}&tablename=b_inward&getisintable=b_isin&getdemattable=b_inward_info`
      );
  ...
  const stgFunction = async (ref, annexure = false) => {
    const documentType = annexure ? "Annexure" : "Inward";
    try {
      const response = await apiConnector(
        "GET",
        `${apiUrl}/v2/admin/process/download-data-of-inward?id=${ref}&type=${documentType}&table=b_inward&isin=b_isin&demattable=b_inward_info`,
        true
      );
      ...
      if (response?.data?.status === 200 || response?.data?.status === true) {
        ...
        if (res2?.data?.status === 200 || res2?.data?.status === true) {
          ...
          // If you're using Zustand, uncomment this:
          ...
          const routeMap = {
            ...
            annexureb: `/admin/inward/annexureb/${encodedRef}`,
          };
  ...
  const inwardForm = async (ref, type) => {
    try {
      ...
      const response = await apiConnector(
        "GET",
        `${apiUrl}${typeToUrl[lowercaseType]}?id=${ref}&&table=b_inward&&isin=b_isin`
      );
