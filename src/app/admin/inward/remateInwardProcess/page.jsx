...
const RemateInwardProcess = () => {
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);
  const apiUrl = process.env.NEXT_PUBLIC_API_BASE_URL;
  const [activeYear, setActiveYear] = useState(null);

  //----------------------fetch RemateInwardProcess data-----------------------------

  const fetchRematProcess = async () => {
    try {
      const response = await apiConnector(
        "GET",
        `${apiUrl}/v2/admin/process/inward/remat-inward-process?table=b_inward&isin=b_isin`
      );
    ...
    <>
      <div className="card">
        <div className="card-body ">
          ...
          {activeYear && (
            <ViewDataTable
              title="Courier Return Inward Process"
              cardHeader="Courier Return Inward Process"
              columns={columns}
...
export default RemateInwardProcess;
